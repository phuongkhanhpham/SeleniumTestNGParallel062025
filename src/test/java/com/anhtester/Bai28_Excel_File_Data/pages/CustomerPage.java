...
public class CustomerPage extends BasePage {
    ...
    public void verifyAddNewCustomerSuccess(int row) {
        ...
        WebUI.assertEquals(DriverManager.getDriver().findElement(inputCompany).getAttribute("value"), excelHelper.getCellData("Customer_Name", row), "The company name is not matched.");
        WebUI.assertEquals(DriverManager.getDriver().findElement(inputVat).getAttribute("value"), excelHelper.getCellData("VAT", row), "The VAT value is not matched.");
        WebUI.assertEquals(DriverManager.getDriver().findElement(inputPhoneNumber).getAttribute("value"), excelHelper.getCellData("Phone", row), "The phone number is not matched.");
        WebUI.assertEquals(DriverManager.getDriver().findElement(inputWebsite).getAttribute("value"), excelHelper.getCellData("Website", row), "The website is not matched.");
    ...
    public void searchAndCheckCustomerInTable(int row) {
        ...
        WebUI.setText(searchCustomer, excelHelper.getCellData("Customer_Name", row));
        WebUI.sleep(2);
        String customerNameInTable = WebUI.getElementText(itemCustomerFirst);
        System.out.println(customerNameInTable);
        WebUI.assertEquals(customerNameInTable, excelHelper.getCellData("Customer_Name", row), "The customer name in table is not matched.");
    }
